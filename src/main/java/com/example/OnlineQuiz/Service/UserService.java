...
import com.example.OnlineQuiz.Exception.EmailNotFoundException;
...
public class UserService {
    ...
    public UserService(UserRepository userRepository, ModelMapper modelMapper, PasswordEncoder passwordEncoder, AuthenticationManager authenticationManager, JwtUtil jwtUtil) {
  ...
  if(dto.getName()!=null && !dto.getName().isEmpty()) {
    ...
    public UserDto UserById(long id) {
        UserEntity user=userRepository.findById(id).orElseThrow(()->new ResourceNotFoundException("userService","USER ID",id,"User Not Found"));
    ...
    public AuthResponseDto checkUser(AuthDto dto) {
        ...
        UserEntity user = userRepository.findByEmail(
                dto.getEmail()
        ).orElseThrow(() -> new EmailNotFoundException("UserService","User ID",dto.getEmail(),"User not found"));
